...
generate_clusters <- function(rows){
  ...
  grouped <- aggregate(time ~ date + m5_turn,
                       data = rows,
                       FUN = mean)
...
communication_aggregate <- data |>
  group_by(sroom,date) |>
  summarise(
    
    participation_entropy = compute_entropy(sparticipant, m4_weight),
    response_entropy      = compute_entropy(sparticipant[-1], m3_response[-1]),
    communication_density = var(m1_past, na.rm = TRUE),
    
    turn_size = sum(m4_weight)/n_distinct(m5_turn) ,
    
    
    
    
    total = sum(m4_weight),
    av_weight    = sum(m4_weight)/n(),
    turns = n_distinct(m5_turn)
