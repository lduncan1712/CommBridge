...
agg_data <- data |>
  group_by(sroom, day) |>
  summarise(
    m1_weight = sum(m4_weight),
    m2_entropy = compute_entropy(sparticipant, m4_weight),
    m3_entropy = compute_entropy(sparticipant[-1], m3_response[-1]),
    m4_varience = var(m1_past[-1],na.rm=TRUE),
    m5_turn = sum(m4_weight)/n_distinct(m5_turn)
  ) 
agg_data
```
...
```{r}
dbExecute(con, "UPDATE communication_aggregate SET day = DATE(day)")
